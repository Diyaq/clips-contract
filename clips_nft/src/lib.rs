...
/// Semantic version string for the contract (#443)
...
pub enum Error {
    ...
    /// Input value failed validation (empty string, symbol too long, etc.)
    InvalidInput = 15,
}
...
pub enum DataKey {
    ...
    Symbol,
    /// Human-readable contract name (#444, instance storage)
    ContractName,
    /// Semantic version string (#443, instance storage)
    ContractVersion,
...
impl ClipsNftContract {
    ...
    pub fn init(env: Env, admin: Address) {
        ...
        // #445: NFT collection name
        env.storage().instance().set(&DataKey::Name, &String::from_str(&env, "ClipCash Clips"));
        // #446: NFT collection symbol
        env.storage().instance().set(&DataKey::Symbol, &String::from_str(&env, "CLIP"));
        // #444: contract name for identification
        env.storage().instance().set(&DataKey::ContractName, &String::from_str(&env, "ClipCashNFT"));
        // #443: semantic contract version
        env.storage().instance().set(&DataKey::ContractVersion, &String::from_str(&env, CONTRACT_VERSION));
    ...
    // -------------------------------------------------------------------------
    // #443: Contract Version Storage
    // -------------------------------------------------------------------------

    /// Returns the stored semantic version string (e.g. `"1.0.0"`).
    ///
    /// Saved at `init` and updated on `upgrade`. Use for compatibility checks
    /// and frontend display instead of the integer `version()`.
    ...
    // -------------------------------------------------------------------------
    // #444: Contract Name Configuration
    // -------------------------------------------------------------------------

    /// Returns the contract name (e.g. `"ClipCashNFT"`).
    pub fn get_contract_name(env: Env) -> String {
        env.storage()
            .instance()
            .get(&DataKey::ContractName)
            .unwrap_or_else(|| String::from_str(&env, "ClipCashNFT"))
    }

    /// Set the contract name. Admin only. Must not be empty.
    pub fn set_contract_name(env: Env, admin: Address, name: String) -> Result<(), Error> {
        Self::require_admin(&env, &admin)?;
        if name.len() == 0 {
    ...
    // -------------------------------------------------------------------------
    // #445: NFT Collection Name Storage
    // -------------------------------------------------------------------------

    /// Returns the NFT collection name (e.g. `"ClipCash Clips"`).
    pub fn get_collection_name(env: Env) -> String {
        env.storage()
            .instance()
            .get(&DataKey::Name)
            .unwrap_or_else(|| String::from_str(&env, "ClipCash Clips"))
    ...
    /// Set the NFT collection name. Admin only. Must not be empty.
    pub fn set_collection_name(env: Env, admin: Address, name: String) -> Result<(), Error> {
        Self::require_admin(&env, &admin)?;
        if name.len() == 0 {
            return Err(Error::InvalidInput);
        }
    ...
    // -------------------------------------------------------------------------
    // #446: NFT Collection Symbol Storage
    // -------------------------------------------------------------------------

    /// Returns the NFT collection symbol (e.g. `"CLIP"`).
    pub fn get_collection_symbol(env: Env) -> String {
        env.storage()
            .instance()
            .get(&DataKey::Symbol)
            .unwrap_or_else(|| String::from_str(&env, "CLIP"))
    ...
    /// Set the NFT collection symbol. Admin only. Must be 1–10 characters.
    pub fn set_collection_symbol(env: Env, admin: Address, symbol: String) -> Result<(), Error> {
        Self::require_admin(&env, &admin)?;
        let len = symbol.len();
        if len == 0 || len > 10 {
            return Err(Error::InvalidInput);
